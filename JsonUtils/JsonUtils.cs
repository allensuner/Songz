...
namespace JsonUtils
{
	...
	public class JsonObject : DynamicObject, IEnumerable, IEnumerator
	{
		...
		public static dynamic GetDynamicJsonObject(JObject jObj)
		{
			return new JsonUtils.JsonObject(jObj);
		}

		public static dynamic GetDynamicJsonObjectFromFile(string jsonFilePath)
		{
			using (StreamReader sr = new StreamReader(jsonFilePath))
			{
				string json = sr.ReadToEnd();
				return GetDynamicJsonObject(json);
			}
		}
		...
		public override bool TrySetMember(SetMemberBinder binder, object value)
		{
			...
			if (obj == null)
			{
				jObject.Add(new JProperty(binder.Name, value));
				return true;
			}
			else
			{
				JObject binderObj = obj as JObject;
				if (binderObj != null)
				{
					binderObj.Replace(JToken.Parse(value.ToString()));
					return true;
				}
				else
				{
					...
					if (jValue != null)
					{
						...
						{
							JContainer parent1 = jValue.Parent;
							JContainer parent2 = parent1.Parent;
							parent1.Remove();
							JProperty prop = new JProperty(binder.Name, value);
							parent2.Add(prop);
							return true;
						}
						else
					...
					{
						//its ok to zero out arrays
						JArray jArray = obj as JArray;
						if (jArray != null && value == null)
						{
		...
		object GetValue(object obj)
		{
			...
			//only convert date times that use mm/dd/yyyy
			if (Regex.IsMatch(val, @"\d\d/\d\d/\d\d\d\d") && DateTime.TryParse(val, out resDateTime)) return resDateTime;
